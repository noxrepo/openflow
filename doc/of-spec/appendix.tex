...
All port state bits are read-only, representing physical link state.
\\\\
The \verb|OFPPS_LINK_DOWN| indicates the the physical link is not present. The \verb|OFPPS_BLOCKED| indicates that a switch protocol outside of OpenFlow, such as 802.1D Spanning Tree or BFD, is preventing the use of that port with \verb|OFPP_FLOOD|.
\\\\
...
\\\\
The metadata field is used to pass information between lookups across multiple tables. This value can be masked using the metadata mask (which is 0xFFFFFFFF by default). If the metadata wildcard bit is set, the contents of these fields are ignored in the lookup.
...
\\\\
The \verb|OFPC_BLOCKED_OVERRIDE| flag indicates that the switch should enable sending and receiving packet through ports in state \verb|OFPPS_BLOCKED|, except when using \verb|OFPP_FLOOD|.
\\\\
