...
\\\\
Setting the \verb|OFPFW_DL_VLAN| or \verb|OFPFW_MPLS_LABEL| bits in the \verb|wildcards| field specifies that a flow should match packets regardless of whether they contain the corresponding tag. Special values are defined below for the VLAN tag and MPLS label to allow the matching of packets with any tag, independent of the tag's value.  Another special value supports matching packets without a VLAN tag, but there is no corresponding value for MPLS packets, since the ethertype can identify whether packets contain an MPLS header. The special values defined for \verb|dl_vlan| include:
...
\\\\
If the \verb|cookie_mask| field is different from 0, it is used with the \verb|cookie| field to restrict flow matching when modifying or deleting flows. This field is ignored by \verb|OFPC_ADD| messages. The \verb|cookie_mask| field's behavior is explained in Section \ref{flow_table:sec_chan:flow_add}.
\\\\
...
\\\\
The \verb|buffer_id| refers to a packet buffered at the switch and sent to the controller by a \verb|OFPT_PACKET_IN| message.  The packet in the switch referenced by the \verb|buffer_id| should be processed by the flow inserted or modified by a following \verb|OFPT_FLOW_MOD| message. This field is ignored by \verb|OFPC_DELETE| and \verb|OFPC_DELETE_STRICT| messages.
\\\\
