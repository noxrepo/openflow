...
\\\\
The metadata field is used to carry information between lookups across multiple tables. This value can be masked using the metadata mask (which is 0xFFFFFFFF by default). If the metadata wildcard bit is set, the contents of these fields are ignored in the lookup (This allows metadata to be carried unchanged across tables).
...
\input{struct/ofp_instruction_metadata}
Metadata for the next table lookup can be written using the \verb|metadata| and the \verb|metadata_mask| in order to set specific bits on the match field. 

\input{struct/ofp_instruction_table_id}
\verb|table_id| indicates the next table to lookup in the packet processing pipeline.
...
\\\\
The \verb|table_id| field specifies the table to put the flow into. Lower
numbered tables are consulted earlier. Table Id 0 signifies the first table 
in the pipeline.
\\\\
...
\\\\
The \verb|priority| field is relevant for all flow entries.  The priority field indicates priority within the specific flow table. Higher numbers indicate higher priorities.
\\\\
