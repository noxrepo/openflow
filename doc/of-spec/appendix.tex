...
Output \qosupd{and enqueue} actions are described in Section~\ref{ft:actions}, tag push/pop actions are described in Table~\ref{table:push pop actions}, and Field-Modify actions are described in Table~\ref{table:field modify actions}.  An action definition contains the action type, length, and any associated data:
...
\\\\
An \verb|action_push_vlan| takes no arguments and consists only of a generic \verb|ofp_action_header|. The action pushes a new VLAN tag onto the packet.
\\\\
...
\\\\
An \verb|action_push_mpls| takes no arguments and consists only of a generic \verb|ofp_action_header|. The action pushes a new MPLS tag onto the packet.
\\\\
An \verb|action_pop_mpls| takes no arguments and consists only of a generic \verb|ofp_action_header|. The action pops the outermost MPLS tag from the packet.
\\\\
