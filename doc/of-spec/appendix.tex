...
The entries are returned in the order that packets traverse the tables. \verb|OFP_MAX_TABLE_NAME_LEN| is \input{define/OFP_MAX_TABLE_NAME_LEN}.
\\\\
...
\\\\
The \verb|match| field indicates the fields for which that particular table supports matching on. For example, a table that support matching on all fields defined by the specification would have it set to \verb|OFPFMF_ALL|.
\\\\
The \verb|wildcards| field indicates the fields for which that particular table supports wildcarding. For example, a direct look-up hash table would have that field set to zero, while a TCAM or sequentially searched table would have it set to \verb|OFPFMF_ALL|.
\\\\
The \verb|match| and \verb|wildcards| fields are set using the following flags :

\input{enum/ofp_flow_match_fields}
