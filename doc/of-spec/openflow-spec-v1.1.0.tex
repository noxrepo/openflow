...
\\\\
OpenFlow ports have limited state such as ``up'', ``down'' and whether spanning tree flood packets should be forwarded out the port.  Additional configuration of ports may handled by the OpenFlow configuration protocol.
...
Each flow entry is associated with a set of instructions that are executed when a packet matches the entry. These instructions result in updates on the action set (TODO: Add link). Supported instructions are:
\begin{itemize} 
...
\label{ft:actionset}
Each flow is associated with an action set. This set is empty by default. A flow entry can interact with the action set using instructions associated with a particular match. The action set is carried between flow tables. When there is no \emph{goto-table} instruction, the actions in the action set are executed. If the action set is empty, the packet will be dropped. The actions maybe executed in arbitrary order except for the output action which is executed last.
\\\\
An action set has a single action of a single type. When multiple actions of the same type are required f.i. pushing multiple MPLS labels or popping multiple MPLS labels, the \emph{apply-action} instruction may be used.
