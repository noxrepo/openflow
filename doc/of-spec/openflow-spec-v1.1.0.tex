...
\\\\
Packets may be matched against one or more flow tables in the switch pipeline. Flow entries may setup actions to modify and forward matching packets or clear actions to drop a packet. Flow entries could also setup metadata that may be matched in the next flow table in the switch pipeline. A packet is forwarded to a single OpenFlow Forwarding ID. This usually indicates a physical port but it could also indicate a variety of forwarding abstractions such as multicast, multipath, flooding (on a VLAN). In addition, there are reserved Forwarding IDs that specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).
\\\\
...
\begin{itemize} 
\item \textbf{Goto-Table \textit{table-id}} : Indicates the next table in the pipeline
\item \textbf{Write-Metadata \textit{metadata}} : Writes the metadata value into the Metadata register
\item \textbf{Write-Actions \textit{action}} : Writes the action(s) into the Action Set
\item \textbf{Apply-Actions \textit{action}} : Applies the action(s) immediately
