...
The OpenFlow protocol provides reliable message delivery, but does not automatically provide message acknowledgements or ensure ordered message processing.  Acknowledgements and ordering can be ensured through the use of \emph{barrier} messages, as described below.  In the absence of barrier messages, switches may arbitrarily reorder messages to maximize performance, hence controllers should not depend on a specific processing order. In particular, flows may be inserted in tables in an order different than that of flow mod messages received by the switch.
\\\\
The barrier message enforces ordering: messages must not be reordered past a barrier message and the barrier message must be processed only when all prior messages have been processed. Consequently, the reply or error messages resulting from OpenFlow messages must not be reordered past the reply to a barrier message.  The controller can use the reply to a barrier message as a confirmation that processing is complete; that is, no switch-generated messages resulting from messages sent prior to the barrier message are pending. If two OpenFlow messages from the controller depend on each other (e.g. a flow add with a following packet-out), they should be separated by a barrier message.
\\\\
Regardless of ordering, switches must process every OpenFlow message sent by a controller in full, possibly generating a reply, while controllers are free to drop messages.  If a switch cannot completely process an OpenFlow message, it must send an error message back to the controller.  For packet-out messages, fully processing the message does not guarantee that the included packet actually exits the switch.  The included packet may be silently dropped after OpenFlow processing due to congestion at the switch, QoS policy, or if sent to a blocked or invalid port.
