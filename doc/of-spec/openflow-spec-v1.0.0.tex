...
\item \textbf{Instruction}: an operation executed when a packet matches a flow entry. An instruction either contains a set of actions or modify pipeline processing.
\item \textbf{Action}: a type of instruction that modifies a packet, including forwarding to a port, decrementing the TTL field, and others. Actions are either part of instructions or action buckets.
...
\item \textbf{Action Bucket}: a set of actions and associated parameters, part of a group.
\end{itemize}
...
\item If the packet has a VLAN header (Ethernet type 0x8100), the VLAN ID and PCP fields are used in the lookup.
