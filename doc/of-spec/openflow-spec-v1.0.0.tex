...
If a group entry with identical ID resides in any table, then that entry, including its counters, must be removed, and the new group entry added.
\\\\
If a switch cannot find any table in which to add the incoming group entry, the switch should send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_ALL_TABLES_FULL| code.
\\\\
If the action list in a group mod message's action buckets references a port that will never be valid on a switch, the switch must return an \verb|ofp_error_msg| with \verb|OFPET_BAD_ACTION| type and \verb|OFPBAC_BAD_OUT_PORT| code.  If the referenced port may be valid in the future, e.g. when a linecard is added to a chassis switch, or a port is dynamically added to a software switch, the switch may either silently drop packets sent to the referenced port, or immediately return an \verb|OFPBAC_BAD_OUT_PORT| error and refuse the group mod.
\\\\
If the action list in a group add or set message references a group that does not exist on a switch, the switch must return an \verb|ofp_error_msg| with \verb|OFPET_BAD_ACTION| type and \verb|OFPBAC_BAD_OUT_GROUP| code.
\\\\
For ADD requests, if a group entry with identical group ID field does not current reside in the group table, the ADD acts like an SET, and the new group entry must be inserted.  Otherwise, the buckets specified in the ADD are added to the existing entry.
\\\\
For DELETE requests, if no group entry with identical ID currently exists in the group table, no error is recorded, and no group table modification occurs.
\\\\
\label{unsupported_flow}If a switch cannot process the action list contained in any group mod message's action bucket in the order specified, it MUST immediately return an \verb|OFPET_GROUP_MOD_FAILED| :
\verb|OFPFMFC_UNSUPPORTED| error and reject the group.

\subsection{Group Removal}

The controller is able to actively remove group entries by sending a group SET message referencing the group ID for removal with no arguments specified.
