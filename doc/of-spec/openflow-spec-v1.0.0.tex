...
\item \textbf{update output port}: if there is an output action, set the packet output port
\item \textbf{group}: apply the actions of the relevant group bucket (group dependant) in order specified by this list
\end{enumerate}

After applying all actions, including iteratively applying any group actions, the packet should be output on the port recorded in the most recent \emph{update output port} step listed above. If the \emph{update output port} step was never applied (ie. the action set and any associated groups did not contain any Forward actions) then the packet should be dropped.
