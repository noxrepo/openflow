...
An OpenFlow Switch consists of a \emph{flow table}, which performs packet lookup and forwarding, a \emph{group table}, which stores forwarding instruction aggregates, and a \emph{secure channel} to an external controller (Figure \ref{fig:flow table and controller}).  The controller manages the switch over the secure channel using the OpenFlow protocol.
\\\\
The flow table contains a set of flow entries (header values to match packets against), activity counters, and a set of zero or more actions to apply to matching packets.  The group table contains a set of group entries (grouped sets of action lists with specific semantics dependent on group type).  All packets processed by the switch are compared against the flow table.  If a matching entry is found, any actions for that entry are performed on the packet (e.g., the action might be to forward a packet out a specified port).  If no match is found, the packet is forwarded to the controller over the secure channel.  The controller is responsible for managing the flow table by adding and removing flow entries.
\\\\
Flow entries may point to one or more groups.   OpenFlow groups can directly represent set of ports for flooding, as well as more complicated methods of forwarding (e.g. multipath, fast reroute, and link aggregation).  As a general layer of indirection, groups also enable multiple flows to forward to a common set of output actions (e.g.  IP forwarding to a common next hop), and for that common set to be changed atomically and efficiently.
\\\\
OpenFlow actions may forward to forwarding identifiers (FIDs), which exist in a 32-bit numberspace.  FIDs include physical ports, virtual ports reserved by a switch, virtual ports reserved by the OpenFlow spec, and groups.  Virtual port examples include tunnels, port aggregates, and VLAN interfaces, as well as reserved ones, including the controller or ingress port (see \ref{ft:actions}).  The phrases 'port' and 'forwarding identifier' are used interchangeably in this document.  Additional port configuration may handled by an external configuration protocol.
...
\end{table}
The ability for one port to point to a \emph{group} of other ports enables OpenFlow to represent additional methods of forwarding (ex. multipath, fast reroute, and link aggregation), as well as to handle current methods (ex. multicast) more efficiently.
\\\\
Each group table entry (see Table \ref{table:group entry}) contains:
\begin{itemize}
\item \textbf{group identifier} to identify the group in the forwarding numberspace
\item \textbf{group type} to determine group semantics (see below)
\item \textbf{action buckets} an ordered list of action buckets, where each action bucket contains a list of actions to execute
\end{itemize}
...
\begin{itemize}
\item \textbf{flood} forward to all buckets in the group.  Packets directed out the input port are dropped by default.  If the controller writer wants to forward out the incoming port, they should include a forward action to the \verb|OFPP_IN_PORT| in each bucket where this is desired.  Used for multicast or broadcast forwarding.
\item \textbf{multipath} forward to one bucket in the group.  Packets are sent to a single bucket in the group, based on a switch-computed selection algorithm (e.g. hash on some user-configured tuple or simple round robin).  All configuration and state for the selection algorithm is external to OpenFlow.  When a port specified in a multipath group goes down, the switch may restrict forwarding to the remaining set of valid ports. This behavior may reduce the disruption of a downed link or switch.
\item \textbf{indirect} forward to one bucket.  Allows multiple flows or groups to point to a common forwarding identifier, supporting faster, more efficient convergence (e.g. next hops for IP forwarding).  This group type is effectively identical to flood groups with one bucket.
\item \textbf{fast failover} forward to the first live bucket.  Each action bucket is associated with a specific port that controls its liveness.  Enables the switch to change forwarding without requiring a round trip to the controller.  If no buckets are live, packets are dropped.  Conditions for liveness are outside the scope of the OpenFlow spec.
\end{itemize}
...
\end{table}Switch designers are free to implement the internals in any way convenient provided that correct functionality is preserved.  For example, while a flow may have multiple forward actions, each pointing to a different group or port, a switch designer may choose to implement this as a single bitmask within the hardware forwarding table.
...
\subsection{Actions}
\label{ft:actions}
Each action bucket in a group or flow entry is associated with zero or more actions that dictate how the switch handles matching packets.  If no forward actions are present, the packet is dropped.  Action lists for \emph{inserted} flow and group entries MUST be processed in the order specified.  However, there is no packet output ordering guaranteed within a port.   For example, an action list may result in two packets sent to two different VLANs on a single port.  These two packets may be arbitrarily re-ordered, but the packet bodies must match those generated from a sequential execution of the actions.
\\\\
A switch may reject a flow entry or group if it cannot process the action list in the order specified, in which case it should immediately return an unsupported flow error (see \ref{unsupported_flow}).  Ordering within a port may vary between vendor switch implementations.
\\\\
...
\\\\
\textbf{Flow-Removed:} When a flow or group entry is added to the switch by a flow or group modify message, an idle timeout value indicates when the entry should be removed due to a lack of activity, as well as a hard timeout value that indicates when the entry should be removed, regardless of activity.  The flow modify message also specifies whether the switch should send a flow or group removed message to the controller when the entry expires.  Flow or group modify messages which delete entries may also cause flow removed messages.
\\\\
...
\\\\
If the action list in a flow mod message references a port that will never be valid on a switch, the switch must return an \verb|ofp_error_msg| with \verb|OFPET_BAD_ACTION| type and \verb|OFPBAC_BAD_OUT_FID| code.  If the referenced port may be valid in the future, e.g. when a linecard is added to a chassis switch, or a port is dynamically added to a software switch, the switch may either silently drop packets sent to the referenced port, or immediately return an \verb|OFPBAC_BAD_OUT_FID| error and refuse the flow mod.
\\\\
...
\\\\
DELETE and DELETE\_STRICT commands can be optionally filtered by destination group or output port.  If the \verb|out_fid| field contains a value other than \verb|OFPP_NONE|, it introduces a constraint when matching.  This constraint is that the rule must contain an output action directed at the specified group or port.  This field is ignored by ADD, MODIFY, and MODIFY\_STRICT messages.
\\\\
...
The action list for each bucket must be validated using the same rules as those for flow mods (Section \ref{flow_table:sec_chan:flow_add}, with additional group-specific checks.  Buckets which consist of a single forward action are allowed. A group may also include buckets which themselves forward to other groups. For example, a fast reroute group may have two buckets, where each bucket forwards to a multipath group. If a switch does not support groups of groups, it must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_CHAINING_UNSUPPORTED| code.
\\\\
For ADD requests, if a group entry with the specified forwarding identifier already resides in the group table, then the switch must refuse to add the group entry and must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_GROUP_EXISTS| code.
\\\\
For MODIFY requests, if a group entry with the specified forwarding identifier already resides in the group table, then that entry, including its type and action buckets, must be removed, and the new group entry added.
\\\\
If a specified forwarding identifier is invalid for group use (reserved by the switch for a port, or one of the special OpenFlow virtual ports (\verb|OFPP_FLOOD|, etc.) then the switch must refuse to add the group entry and must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_INVALID_GID| code.
\\\\
If a switch does not support unequal load sharing with multipath groups, it must refuse to add the group entry and must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_NON_EQUAL_MP_UNSUPPORTED| code.
\\\\
...
\\\\
If a switch cannot add the incoming group entry due to restrictions (hardware or otherwise) limiting the number of group members, is must refuse to add the group entry and must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_OUT_OF_MP_MEMBERS| code.
\\\\
For DELETE requests, if no group entry with the specified identifier currently exists in the group table, no error is recorded, and no group table modification occurs.  Otherwise, the group is removed, and all flows that forward to the group are also removed.  DELETE differs from an ADD or MODIFY with no buckets specified in that future attempts to ADD to the forwarding identifier will not result in a group exists error.  If one wishes to effectively delete a group yet leave in flow entries using it, that group can be cleared by sending a MODIFY with no buckets specified.
\\\\
To delete all groups with a single message, specify \verb|OFPP_GROUP_ALL| as the group value.
