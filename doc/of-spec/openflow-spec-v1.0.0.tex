...
An instruction set contains a maximum of one instruction of each type. The instructions of the set executed in the order specified by this above list, in practice the only constraint is that the Clear-Actions instruction is executed before the Write-Actions instruction.
\\\\
...
The output action in the action set is executed last. If both a output action and a group action are specified in an action set, the output action is ignored and the group action takes precedence. If no output action and no group action were specified in an action set, the packet is dropped. The execution of group is recursive ; a group bucket may specify an other group, in which case the execution of actions traverse all the groups specified by the group configuration.

\subsection{Action List}
\label{ft:actionlist}

The \textit{Apply-Action} instruction and the \textit{Packet-out} message include an action list. The semantic of the action list is identical to the OpenFlow 1.0 specification. The actions of an action list are executed in the order specified by the list, and are applied immediately to the packet.
\\\\
...
\\\\
After the execution of the action list in a \textit{Apply-Action} instruction, pipeline execution continues, the packet may be matched by subsequent tables as needed and the packet instruction set is executed as needed. The instruction set is unchanged by the execution of the action list.
