...
\\\\
The OpenFlow port space includes virtual ports, which may represent tunnels, port aggregates, or VLAN interfaces, as well as regular physical ports.  OpenFlow ports have limited state such as ``up'', ``down'' .  Additional configuration of ports may handled by the OpenFlow configuration protocol.  There are several OpenFlow virtual ports used to indicate, for example, the controller or ingress port (see \ref{ft:actions}).
