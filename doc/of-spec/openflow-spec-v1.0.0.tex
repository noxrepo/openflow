...
\\\\
Flow entries may forward to one or more ports, which exist in a 32-bit space.  OpenFlow ports include physical ports, virtual ports reserved by a switch, and virtual ports reserved by the OpenFlow spec.  Virtual port examples include tunnels, port aggregates, and VLAN interfaces, as well as reserved ones, including the controller or ingress port (see \ref{ft:actions}).  
\\\\\
OpenFlow groups exist in a separate 32-bit space.  Groups can directly represent sets of ports for flooding, as well as more complicated methods of forwarding (e.g. multipath, fast reroute, and link aggregation).  As a general layer of indirection, groups also enable multiple flows to forward to a common set of output actions (e.g.  IP forwarding to a common next hop), and for that common set to be changed atomically and efficiently.  
\\\\
Additional port/group configuration may be handled by an external configuration protocol.
...
\begin{itemize}
\item \textbf{group identifier} to identify the group
...
\\\\
\textbf{Required Action:} \emph{Group}.  Apply the group to the current packet.  The exact interpretation depends on the specific group type. 
...
\\\\
If a specified group identifier is invalid for group use (reserved by the switch for a port, or one of the special OpenFlow virtual ports (\verb|OFPP_FLOOD|, etc.) then the switch must refuse to add the group entry and must send an \verb|ofp_error_msg| with \verb|OFPET_GROUP_MOD_FAILED| type and \verb|OFPGMFC_INVALID_GROUP| code.
\\\\
