...
<<<<<<< HEAD:doc/of-spec/openflow-spec-v1.0.0.tex
...
\\\\
Each flow table in the switch contains a set of flow entries; each flow entry consists of header values to match packets against, activity counters, and a set of instructions to apply to matching packets.  The group table contains group entries; each group entry includes a list of action buckets with specific semantics dependent on group type.
\\\\
Flow entries match packets in priority order. If a matching entry is found, the instructions associated with the specific flow entry are executed. If no match is found in a flow table, the packet is forwarded to the controller over the secure channel. The controller is responsible for determining how to handle packets without valid flow entries, and it manages the switch flow tables by adding, removing, and updating flow entries.
\\\\
...
\\\\
Packets are forwarded to ports. This usually indicates a physical port but it could also indicate a variety of virtual ports such as multicast, multipath and fast-failover. In addition, there are reserved virtual ports that specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).
\\\\
...
\item \textbf{indirect}: Execution the one defined bucket in this group.  Allows multiple flows or groups to point to a common group identifier, supporting faster, more efficient convergence (e.g. next hops for IP forwarding).  This group type is effectively identical to flood groups with one bucket.
...
\begin{figure}[!htb]
\centering
\includegraphics[height=1.6in]{packet_flow_flowchart}
...
\\\\
To handle the various Ethernet framing types, matching the Ethernet type is handled in a slightly different manner.  If the packet is an Ethernet II frame, the Ethernet type is handled in the expected way.  If the packet is an 802.3 frame with a SNAP header and Organizationally Unique Identifier (OUI) of 0x000000, the SNAP protocol id is matched against the flowÕs Ethernet type.  A flow entry that specifies an Ethernet type of 0x05FF, matches all Ethernet 802.2 frames without a SNAP header and those with SNAP headers that do not have an OUI of 0x000000.  
\\\\
