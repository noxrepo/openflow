...
An OpenFlow Switch consists of one of more \emph{flow tables}, which perform packet lookups and forwarding. It also consists of a \emph{secure channel} to an external controller (Figure \ref{fig:flow table and controller}).  The controller manages the switch over the secure channel using the OpenFlow protocol.
\\\\
Each flow table contains a set of flow entries that are organized in priority order. If a matching entry is found, the instructions associated with the specific flow entry are executed. If no match is found in a flow table, the packet is forwarded to the controller via the secure connection. The controller is responsible for determining how to handle packets without valid flow entries, and it manages the switch flow tables by adding, removing and updating flow entries.
\\\\
Packets are always matched against table 0 and may be matched against more flow tables in the switch pipeline. Flow entries may setup actions to modify and forward matching packets or clear actions to drop a packet. Flow entries could specify the next table in the pipeline as well as metadata that may be matched in that table. If the entry does not indicate the next table in the pipeline, the packet is processed. A packet is forwarded to a port. This usually indicates a physical port but it could also indicate a variety of virtual ports such as multicast, multipath and fast-failover. In addition, there are reserved virtual ports that specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).
\\\\
OpenFlow ports have limited state such as ``up'', ``down'' and whether spanning tree flood packets should be forwarded out the port.
...
\begin{itemize} 
\item \textbf{match fields} to match against packets. These include packet headers as well the ingress port and metadata (specified by a previous table). 
\item \textbf{counters} to update for matching packet 
\item \textbf{instructions} to apply to the action set on matching packets 
\end{itemize} 
...
Table \ref{table:header fields} shows the header fields an incoming packet is compared against. Each entry contains a specific value, or ANY, which matches any value. If the switch supports subnet masks on the IP source and/or destination fields, these can more precisely specify matches.  The fields in the OpenFlow 12-tuple are listed in Table \ref{table:header fields} and details on the properties of each field are described in Table \ref{table:header field details}. In addition to packet headers, matches can also be performed against ingress port and metadata. Metadata (implementation dependent) is used to signal lookup information between tables in a switch (starting from table 1).
\\\\
\begin{table}[hbp]
\centering
\footnotesize
...
\hline Metadata & (Implementation dependent) & On table 1 and above & \\
...
\hline Invalid TTL & 8 & All IP and MPLS packets & This will match TTL=1 and TTL=0 packets \\
...
\end{table}Switch designers are free to implement the internals in any way convenient provided that correct functionality is preserved. For example, a flow may forward to a Forwarding ID that eventually forwards to multiple ports, a switch designer may choose to implement this as a single bitmask within the hardware forwarding table.
...
Each flow entry is associated with a set of instructions that are executed when a packet matches the entry. These instructions result in updates on the action set (see: \ref{ft:actionset}). Supported instructions are:
\begin{itemize} 
\item \textbf{Goto-Table \textit{table-id}} : Indicates the next table in the pipeline. The table-id must be greater than the current table-id.
\item \textbf{Write-Metadata \textit{metadata / mask}} : Writes the masked metadata value into the Metadata register.
\item \textbf{Write-Actions \textit{action(s)}} : Writes the action(s) into the Action Set
\item \textbf{Apply-Actions \textit{action(s)}} : Applies the action(s) immediately without any change to the Action Set.
\item \textbf{Clear-Actions} : Clears all the actions in the Action Set. This is also executed immediately.
\end{itemize} 

A switch may reject a flow entry if it is unable to execute the instructions associated with the flow entry. In this case, the switch must return an unsupported flow error (see \ref{unsupported_flow}). Every flow table is not expected to support every match ane every instruction.

\subsection{Action Set}
\label{ft:actionset}
Each packet that matches a flow entry is associated with an action set. This set is empty by default. A flow entry can interact with the action set using instructions associated with a particular match. The action set is carried between flow tables. When there is no \textit{Goto-Table} instruction, the actions in the action set are executed. If the action set is empty, the packet will be dropped. The actions maybe executed in arbitrary order except for the output action which is executed last.
\\\\
An action set has a single action of a single type. When multiple actions of the same type are required f.i. pushing multiple MPLS labels or popping multiple MPLS labels, the \emph{Apply-Actions} instruction may be used.
\\\\
If a packet does not match any entry in a flow table, it is forwarded to the controller. The controller can forward or drop the packet. It can also respond by installing appropriate flow entries so the switch can match and handle the packet. The controller can also prevent this default behavior by installing a catch-all low priority flow entry that drops packets that don't match any flow entries in the table.
...
\subsection{Actions}
\label{ft:actions}
Each packet that matches a flow entry is associated with an action set \ref{ft:actionset}. The action set contains zero or more actions that dictate what the switch does on the packet. This action set is executed when there is no \textit{Goto-Table} instruction. If no forward actions are present, the packet is dropped. Otherwise, the packet maybe modified and forwarded. Action sets are processed in arbitrary order except for the output action which is executed last.
\\\\
