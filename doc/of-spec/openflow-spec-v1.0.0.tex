...
\\
Note: The default value is zero for \emph{all} fields when pushing a new header. Field values should be overridden using Modify-Field actions.
\\\\
...
Replace the existing MPLS label field.
This action is only applied to MPLS packets. \\
\hline
Modify MPLS traffic class &
3 bits: Value with which to replace existing MPLS traffic class field &
Replace the existing MPLS traffic class field.
This action is only applied to MPLS packets. \\
\hline
Modify MPLS TTL &
8 bits: Value with which to replace existing MPLS TTL field &
Replace the existing MPLS TTL field.
This action is only applied to MPLS packets. \\
\hline
Decrement MPLS TTL &
- &
Decrement the MPLS TTL field.
This action is only applied to MPLS packets. \\
\hline
