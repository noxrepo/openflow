...
The OpenFlow protocol provides reliable message delivery, but does not automatically provide message acknowledgements or ensure ordered message processing.  Acknowledgements and ordering can be ensured through the use of \emph{barrier} messages, as described below.  In the absence of barrier messages, switches may arbitrarily reorder messages to maximize performance, hence controllers should not depend on a specific processing order. In particular, flows may be inserted in tables in an order different than that of flow mod messages received by the switch.  Messages are guaranteed delivery, unless the connection fails entirely, in which case the controller should not assume anything about the switch state (for example, the switch may have gone into ``fail standalone mode``).
\\\\
The barrier message enforces ordering: messages must not be reordered past a barrier message and the barrier message must be processed only when all prior messages have been processed. Consequently, the reply or error messages resulting from messages must not be reordered past the reply to a barrier message.  The controller can use the reply to a barrier message as a confirmation that processing is complete; that is, no switch-generated messages resulting from messages sent prior to the barrier message are pending. If two messages from the controller depend on each other (e.g. a flow add with a following packet-out), they should be separated by a barrier message.
\\\\
Regardless of ordering, switches must process every message sent by a controller in full, possibly generating a reply.  If a switch cannot completely process a message, it must send an error message back to the controller.  In addition, switches must send to the controller all asynchronous messages generated by internal state changes, such as flow-removed messages.  For packet-out messages, fully processing the message does not guarantee that the included packet actually exits the switch.  The included packet may be silently dropped after OpenFlow processing due to congestion at the switch, QoS policy, or if sent to a blocked or invalid port.  Controllers are free to drop messages, with the exception of hello and echo messages; these are necessary to prevent the switch from dropping the connection.
