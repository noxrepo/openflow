...
\subsection{Message delivery and processing}

The OpenFlow protocol does not provide explicit acknowledgements for the various OpenFlow messages, the protocol construction make this unnecessary. The OpenFlow protocol runs over a reliable channel, OpenFlow messages are guaranteed to be delivered, unless the connection fails entirely. The controller may ignore any OpenFlow message sent by the switch without adverse effect on the protocol correctness. On the other hand, the switch must process every OpenFlow message sent by the controller. The switch must not silently drop an openflow command, if it can not be applied a meaningful error message must be sent back to the controller.
\\\\
Any \emph{modify-State} message must modify the required state on the switch or return an error. Any \emph{read-State} message must return the required state to the controller or return an error. Any \emph{packet-out} message must also be processed by the switch, however the packet included in the \emph{packet-out} message may be silently dropped after OpenFlow processing and prior to forwarding due to an congestion at the switch, QoS policy or if sent to a blocked or invalid port.
\\\\
OpenFlow messages are not ordered, the switch may reorder OpenFlow message arbitrarily to maximise the switch processing performance, and the controller should not depend on a specific processing order. In particular, flows may be inserted in the tables in an different order from the way they are delivered at the switch.
\\\\
The \emph{barrier} message enforce ordering, OpenFlow messages can not be reordered across a \emph{barrier} message and the barrier message can not be processed until all message prior to it are processed. Consequently, the reply or error messages resulting from those OpenFlow messages can not be reordered across the reply to a \emph{barrier} message, and the controller can use the reply to a \emph{barrier} message as a confirmation that processing is complete and no more message pertaining to those messages prior to the \emph{barrier} message are pending. If two OpenFlow messages from the controller depend on each other (for example when \verb|OFPP_TABLE|), they should be separated by a \emph{barrier} message.
