...
\\\\
When the actions of an action set are executed, they are applied to the packet in the order specified below, irrespective of the order they were added to the set. The output action in the action set is executed last.

\begin{enumerate}
\item \textbf{pop}: all tag pop actions
\item \textbf{push}: all tag push actions
\item \textbf{set}: all modify-field actions
\item \textbf{qos}: all QoS actions, such as set\_queue
\item \textbf{output}: the output action (physical or virtual port)
\end{enumerate}
