...
\\\\
Packets are forwarded to ports. This is usually a physical port but it could also be a group to support forwarding behaviors such as multicast, multipath and fast-failover. In addition, there are reserved virtual ports that specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).
\\\\
...
\section{Glossary}
This section describes key terms defined in this specification.
\begin{itemize}
\item \textbf{Match Fields}: are the list of fields that the packet is matched against. These include packet headers, ingress ports and metadata values.
\item \textbf{Metadata}: is a maskable register value that can be used to carry information from one table to the next.
\item \textbf{Instructions}: are operations that are executed when a packet matches a flow entry.
\item \textbf{Actions}: dictate how the switch handles the packet, for instance, forward to a port, decrement ttl etc.
\item \textbf{Action Set}: is a set of actions that are carried between tables along with the match fields.
\item \textbf{Group}: consists of a list of action buckets and some means of choosing one or more of those buckets to apply on a per-packet basis.
\item \textbf{Action Bucket}: is a set of actions that represents a unit within a group.
\end{itemize}
...
Table \ref{table:header fields} shows the match fields an incoming packet is compared against. Each entry contains a specific value, or ANY, which matches any value. If the switch supports subnet masks on the IP source and/or destination fields, these can more precisely specify matches.  The fields in the OpenFlow n-tuple are listed in Table \ref{table:header fields} and details on the properties of each field are described in Table \ref{table:header field details}. In addition to packet headers, matches can also be performed against the ingress port and metadata fields. Metadata may be used to pass information between tables in a switch.
\\\\
\begin{table}[hbp]
\centering
\footnotesize
...
\label{ft:actionset}
Each packet that matches a flow entry is associated with an action set. This set is empty by default. A flow entry can interact with the action set using instructions associated with a particular match. The action set is carried between flow tables. When there is no \textit{Goto-Table} instruction, the actions in the action set are executed. If the action set does not contact an output action, the packet will be dropped. The actions may be executed in arbitrary order, except for the output action, which is executed last.
\\\\
