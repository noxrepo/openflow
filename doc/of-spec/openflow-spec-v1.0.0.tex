...
\\\\
For MODIFY requests, if a flow entry with identical match fields does not currently reside in the requested table, and if the \verb|cookie_mask| field contains 0, the MODIFY acts like an ADD, and the new flow entry must be inserted with zeroed counters.  If a matching entry exists in the table, only the \verb|instructions| field is updated with the value from the request, whereas its \verb|cookie|, \verb|idle_timeout|, \verb|hard_timeout|, \verb|flags|, counters and duration fields are left unchanged.
\\\\
...
\\\\
If the instructions requested in a flow mod message are unsupported, or if the instructions contain a Goto-Table and the next-table-id refers to an invalid table, the switch must return an \verb|ofp_error_msg| with \verb|OFPET_FLOW_MOD_FAILED| type and \verb|OFPFMC_BAD_INSTRUCTION| code.
\\\\
