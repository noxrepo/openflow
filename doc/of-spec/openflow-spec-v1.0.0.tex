...
\\\\
Flow entries may forward to a \emph{port}. This is usually a physical port, but it may also be a logical port defined by the switch or a virtual port defined by this specification. Virtual ports may specify generic forwarding actions such as sending to the controller, flooding, or forwarding using non-OpenFlow methods, such as ``normal'' switch processing (see \ref{ft:actions}), logical ports may specify link aggregation groups, tunnels or loopback interfaces (see \ref{ft:actions}).
\\\\
