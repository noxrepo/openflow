...
\\\\
For DELETE requests, if no group entry with the specified identifier currently exists in the group table, no error is recorded, and no group table modification occurs.  Otherwise, the group is removed.   DELETE differs from an ADD or MODIFY with no buckets specified in that future attempts to ADD to the forwarding identifier will not result in a group exists error.
