...
An OpenFlow Switch consists of a \emph{flow table} and a \emph{group table}, which perform packet lookup and forwarding, as well as a \emph{secure channel} to an external controller (Figure \ref{fig:flow table and controller}).  The controller manages the switch over the secure channel using the OpenFlow protocol.
\\\\
The flow table contains a set of flow entries (header values to match packets against), activity counters, and a set of zero or more actions to apply to matching packets.  The group table contains group entries ( a list of action buckets with specific semantics dependent on group type).  All packets processed by the switch are compared against the flow table.  If a matching entry is found, any actions for that entry are performed on the packet (e.g., the action might be to forward a packet out a specified port).  If no match is found, the packet is forwarded to the controller over the secure channel.  The controller is responsible for determining how to handle packets without valid flow entries, and it manages the switch flow table by adding and removing flow entries.
\\\\
Flow entries may forward to single ports.  OpenFlow ports include physical ports, virtual ports reserved by a switch, and virtual ports reserved by the OpenFlow spec.  Virtual ports may include, but are not limited to, tunnels, port aggregates, VLAN interfaces, and OpenFlow-spec-defined special ports such as the controller or ingress port (see \ref{ft:actions}).  
\\\\\
...
\subsection{Actions}
\label{ft:actions}
Each action set in a bucket in a flow entry group action bucket is associated with zero or more actions that dictate how the switch handles matching packets.  If no forward actions are present, the packet is dropped.  Action sets for \emph{inserted} flow and group entries MUST be processed in the order specified.  However, there is no packet output ordering guaranteed within a port.   For example, a use of flood groups  may result in two packets sent to two different VLANs on a single port.  These two packets may be arbitrarily re-ordered, but the packet bodies must match those generated from a sequential execution of the actions.
\\\\
