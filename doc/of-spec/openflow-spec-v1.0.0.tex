...
\\\\
Flow entries may forward to a \emph{port}. This is usually a physical port, but it may also be a logical port defined by the switch or a virtual port defined by this specification. Virtual ports may specify generic forwarding actions such as sending to the controller, flooding, or forwarding using non-OpenFlow methods, such as ``normal'' switch processing (see \ref{ft:actions}), logical ports may define a tunnel interfaces or a loopback (see \ref{ft:actions}).
\\\\
...
\item \textbf{Pipeline}: how OpenFlow process packets.
\item \textbf{Port}: where packets enter and exit the OpenFlow pipeline. May be a physical port, a logical port defined by the switch or a virtual port defined by the OpenFlow protocol. Virtual ports are ports reserved ports by this specification (see \ref{ft:actions}). Logical ports are higher level abstractions that may be defined in the switch using non-OpenFlow methods (e.g. link aggregation groups, tunnels, loopback interfaces).
...
\hline Ingress Port & 32 & All packets & Numerical representation of incoming port, starting at 1. This may be a physical or logical port as defined by the switch implementation. \\
...
The Set-Output-Port action forwards a packet to a specified port. OpenFlow switches must support forwarding to the physical ports and logical ports defined by the switch. Standard ports are defined as physical ports, logical ports and the \verb|LOCAL| port if supported (excluding other virtual ports). OpenFlow switches must support forwarding to the following virtual ports:
\begin{itemize}
