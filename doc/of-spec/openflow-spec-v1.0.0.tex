...
\begin{sideways}Meta data \end{sideways} &
\begin{sideways}Ether source \end{sideways} &
...
The currently supported ordering of header fields/tags is:
\begin{center}
...
The ethertype is used as the ethertype for the tag. Only well-known values for ethertype should be used.
\\
\hline
Pop VLAN header &
- &
...
The ethertype is used as the ethertype for the header only if the new tag being pushed is the bottom of stack. Only well-known values for ethertype should be used.
\\
\hline
Pop MPLS header &
Ethertype &
...
\hline
Set MPLS label &
20 bits: Value with which to replace existing MPLS label field &
Replace the existing MPLS label.
Only applies to packets with an existing MPLS shim header. \\
\hline
Set MPLS traffic class &
3 bits: Value with which to replace existing MPLS traffic class field &
Replace the existing MPLS traffic class.
Only applies to packets with an existing MPLS shim header. \\
\hline
Set MPLS TTL &
8 bits: Value with which to replace existing MPLS TTL field &
