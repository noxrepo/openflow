...
\\\\
Matching starts at the first table and may continue to additional tables. Flow entries may include instructions to modify/forward the packets and/or alter the pipeline processing.  Flow entries may also specify the next table in the pipeline as well as metadata that may be matched in that table. Table pipeline processing is stopped when a next table is not specified.
\\\\
Flow entries may forward to a port. Usually, this is a physical port, but it could also be a virtual port reserved by a switch or by this specification. Virtual ports may specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).  The zero port is reserved and cannot be used for forwarding.
\\\\
...
A group table consists of group entries.  The ability for a flow to point to a \emph{group} enables OpenFlow to represent additional methods of forwarding (e.g. multipath and multicast).
\\\\
\begin{table}[hbp]
\centering
\begin{tabular}{|c|c|c|}
...
\\\\
\textbf{Read-State:} Read-State messages are used by the controller to collect statistics from the switch’s flow-tables, group-table, ports and the individual flow entries.
\\\\\
