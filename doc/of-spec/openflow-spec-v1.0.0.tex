...
\\\\
The \emph{barrier} message enforce ordering, OpenFlow messages can not be reordered across a \emph{barrier} message and the barrier message can not be processed until all message prior to it are processed. If two OpenFlow messages from the controller depend on each other, they should be separated by a \emph{barrier} message.
