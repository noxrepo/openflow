...
\\\\
When the actions of an action set are executed, they are applied to the packet in the order specified below, irrespective of the order they were added to the set. The output action in the action set is executed last, unless there is a group action. The group action applies this process recursively using the action set specified by the group, which may itself contain a group action. When an action set specifies a group action, in general the output action of the same set is ignored, the exception is when the action set of the relevant group bucket does not specify an output action itself.

\begin{enumerate}
\item \textbf{copy TTL inwards}: apply all copy TTL inwards actions to the packet
\item \textbf{pop}: apply all tag pop actions to the packet
\item \textbf{push}: apply all tag push actions to the packet
\item \textbf{copy TTL outwards}: apply all copy TTL outwards actions to the packet
\item \textbf{decrement TTL}: apply all decrement TTL actions to the packet
\item \textbf{set}: apply all set-field actions to modify the packet
\item \textbf{qos}: apply all QoS actions, such as set\_queue to the packet
\item \textbf{set-output}: if there is an output action, set the packet output port
\item \textbf{do-output}: if the set does not contain a group action, and if the packet output port is set, forward the packet on the port specified (physical or virtual port)
\item \textbf{group}: if the set contains a group action, recursively apply the action set defined by the relevant group bucket (group dependant)
\end{enumerate}

If the switch supports it, it is possible to execute actions in a different order by using a set of \textit{Apply-Actions} instructions.
