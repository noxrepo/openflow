...
An OpenFlow Switch consists of one or more \emph{flow tables} and a \emph{group table}, which perform packet lookups and forwarding, as well as a \emph{secure channel} to an external controller (Figure \ref{fig:flow table and controller}).  The controller manages the switch over the secure channel using the OpenFlow protocol.
\\\\
...
\\\\
Flow entries match packets in priority order. If a matching entry is found, the instructions associated with the specific flow entry are executed. If no match is found in a flow table, the packet is forwarded to the controller over the secure channel (or dropped based on configuration). The controller is responsible for determining how to handle packets without valid flow entries, and it manages the switch flow tables by adding, removing, and updating flow entries.
\\\\
Matching starts at the first table and may continue to additional tables. Flow entries may include actions to modify and/or forward the packet, as well as instructions to modify the pipeline processing.  Flow entries may also specify the next table in the pipeline as well as metadata that may be matched in that table. Table pipeline processing is stopped when a next table is not specified.
\\\\
Flow entries may forward to a port. Usually, this is a physical port, but it could also be a virtual port reserved by a switch or by the OpenFlow spec. Virtual ports may specify generic forwarding actions such as sending to the controller, flooding or normal forwarding using non-OpenFlow methods (see \ref{ft:actions}).  The zero port is reserved and cannot be used for forwarding.
\\\\
Groups can directly represent sets of ports for flooding, as well as more complex forwarding semantics (e.g. multipath, fast reroute, and link aggregation).  As a general layer of indirection, groups also enable multiple flows to forward to a single identifier (e.g. IP forwarding to a common next hop).  This abstraction allows common output actions across flows to be changed atomically and efficiently.
\\\\
...
\item \textbf{Metadata}: a maskable register value that can be used to carry information from one table to the next.
\item \textbf{Instruction}: an operation executed when a packet matches a flow entry
\item \textbf{Action}: a type of instruction that modifies a packet, including forwarding to a port, decrementing the TTL field, and others.
\item \textbf{Action Set}: a set of actions that are carried between tables along with the match fields.
...
\item \textbf{Action Bucket}: a set of actions and associated parameters.
\end{itemize}
...
\begin{itemize} 
\item \textbf{match fields}: to match against packets. These include the ingress port, packet headers, and may include metadata specified by a previous table.
...
\hline
\end{tabular}
\caption{A group entry consists of a group identifier, group type and a list of action buckets.}
\label{table:group entry}
\end{table}
A group table consists group entries.  The ability for a flow to point to a \emph{group} enables OpenFlow to represent additional methods of forwarding (e.g. multipath and multicast).
\\\\
...
\item \textbf{group type}: to determine group semantics
...
\begin{itemize}
\item \textbf{flood}: Execute all buckets in the group.  Packets directed out the ingress port are dropped by default.  If the controller writer wants to forward out the ingress port, they should include a forward action to the \verb|OFPP_IN_PORT| in each bucket where this is desired.  This group is used for multicast or broadcast forwarding.
...
\item \textbf{fast failover}: Execute the first live bucket.  Each action bucket is associated with a specific port and/or group that controls its liveness.  Enables the switch to change forwarding without requiring a round trip to the controller.  If no buckets are live, packets are dropped.  Conditions for liveness are outside the scope of the OpenFlow spec.
\end{itemize}

All \textit{fast failover groups} must support a liveness mechanism, where the group continuously watch the liveness of other ports and or groups to determine which bucket to use to process packets. Other group types are not required to implement the liveness mechanism, but may optionally implement it. When a port or a group of any type is watched by a \textit{fast failover group}, it must implement the liveness mechanism ; if the implementation can not support it it must refuse to install the \textit{fast failover group} and return an error.
\\\\
The \textbf{liveness mechanism} must follow those rules :
\begin{itemize}
\item 
A port is considered \textit{live} if the state of the port contains the flag \verb|OFPPS_LIVE|. Port liveness is hardware dependant and managed by the switch, at a minimum, the port config should not contain \verb|OFPPC_PORT_DOWN| and its state should not contain \verb|OFPPS_LINK_DOWN|, the switch may also use Spanning Tree and some KeepAlive mechanism to determine port liveness.
\item A bucket is considered \textit{live} if either the port or the group watched by the bucket are live.
\item A group is considered \textit{live} if a least one of its bucket is live.
\end{itemize}
...
Table \ref{table:match fields} shows the match fields an incoming packet is compared against. Each entry contains a specific value, or ANY, which matches any value. If the switch supports subnet masks on the IP source and/or destination fields, these can more precisely specify matches.  The fields in the OpenFlow n-tuple are listed in Table \ref{table:match fields} and details on the properties of each field are described in Table \ref{table:header field details}. In addition to packet headers, matches can also be performed against the ingress port and metadata fields. Metadata may be used to pass information between tables in a switch.
\\\\
\begin{table}[hbp]
\centering
\footnotesize
...
\hline Metadata & 64 & On table 1 and above & \\
...
\end{table}Switch designers are free to implement the internals in any way convenient provided that correct functionality is preserved. For example, while a flow may use a multicast group to forward to multiple ports, a switch designer may choose to implement this as a single bitmask within the hardware forwarding table.
...
Each flow entry is associated with a set of instructions that are executed when a packet matches the entry. These instructions result in changes to the action set and/or pipeline processing (see: \ref{ft:actionset}). Supported instructions include:
\begin{itemize} 
...
\item \textbf{Write-Actions \textit{action(s)}}: Merges the specified action(s) into the current action set.  If the action exists in current set, overwrite it, otherwise add it.
\item \textbf{Apply-Actions \textit{action(s)}}: Applies the specific action(s) immediately, without any change to the Action Set.
\item \textbf{Clear-Actions}: Clears all the actions in the action set immediately.
\end{itemize} 

A switch may reject a flow entry if it is unable to execute the instructions associated with the flow entry. In this case, the switch must return an unsupported flow error (see \ref{unsupported_flow}).  Flow tables may not support every match and every instruction.

\subsection{Action Set}
\label{ft:actionset}
Each packet that matches a flow entry is associated with an action set. This set is empty by default. A flow entry can interact with the action set using instructions associated with a particular match. The action set is carried between flow tables. When there is no \textit{Goto-Table} instruction, the actions in the action set are executed. If the action set does not contain an output action, the packet will be dropped. The output action in the action set is executed last.
\\\\
An action set has a single action of each type. When multiple actions of the same type are required, e.g. pushing multiple MPLS labels or popping multiple MPLS labels, the \emph{Apply-Actions} instruction may be used.
\\\\
If a packet does not match any entry in a flow table, it is forwarded to the controller. The controller can forward or drop the packet. It can also respond by installing appropriate flow entries so the switch can match and handle the packet. The controller can also prevent this default behavior by installing a catch-all low priority flow entry, dropping unmatched packets.
...
\begin{itemize}
\item \textbf{ALL:} Send the packet out all physical ports, including \verb|LOCAL| port, and not including the ingress port and ports that are configured \verb|OFPPC_NO_FWD|.
...
\item \textbf{FLOOD:} Send the packet out all physical ports, including \verb|LOCAL| port, and not including the ingress port, ports that are configured \verb|OFPPC_NO_FWD| and ports that are in \verb|OFPPS_BLOCKED| state. The switch may optionally restrict forwarding to the VLAN matching the packet VLAN header field.
\end{itemize}
...
\\\\
For valid (non-overlapping) ADD requests, or those with no overlap checking, the switch must insert the flow entry at the lowest numbered table for which the switch supports all wildcards set in the \verb|flow_match| struct, and for which the priority would be observed during the matching process.  If a flow entry with identical match fields and priority already resides in any table, then that entry, including its counters, must be removed, and the new flow entry added.
\\\\
...
\\\\
If the action list in a flow mod message references a group that is not currently defined on the switch, or will never be valid, such as \verb|OFPG_ALL|, the switch must return an \verb|ofp_error_msg| with \verb|OFPET_BAD_ACTION| type and \verb|OFPBAC_BAD_OUT_GROUP| code.
\\\\
...
\\\\
MODIFY, MODIFY\_STRICT, DELETE and DELETE\_STRICT commands can also be filtered by cookie value, if the \verb|cookie_mask| field contains a value other than 0. This constraint is that the bits specified by the \verb|cookie_mask| in both the \verb|cookie| field of the flow mod and a flow's \verb|cookie| value must be equal. In other words, (flow.cookie \& flow_mod.cookie_mask) == (flow_mod.cookie \& flow_mod.cookie_mask).
\\\\
